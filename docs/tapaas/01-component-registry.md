...
| MPS medical evidence status | `0:17316`, `0:17333`; boundary nodes `0:17327`, `0:17344`, `0:17351` | TaPaaS-specific evidence-status pattern | coded-preview | Mobility Parking Scheme evidence/medical stage | Preview component: `MpsMedicalEvidenceStatusPreview`. Preserves static required/provided evidence-status anatomy with mock file-name display only. Source upload-state and file-limit inconsistencies remain review-gated. |
...
| `MpsMedicalEvidenceStatusPreview` | `state`, optional `fileName` | MPS Medical certificate frames `0:17316`, `0:17333`; boundary nodes `0:17327`, `0:17344`, `0:17351` | Preview-only medical evidence status pattern. Reuses `EvidenceChecklistCard` and `InPageAlert`; shows required/provided states and a static mock file name only. No upload, remove-file behaviour, storage, validation, virus scanning, backend integration, medical assessment or privacy/security approval. |

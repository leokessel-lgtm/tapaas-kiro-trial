...
| Accordion | `Accordion` GEL preview component | GEL-aligned | needs engineer review | Source: `@snsw-gel/accordion` (evidence at `docs/source-evidence/gel-components/accordion/`). Button-based headings with `aria-expanded` and `aria-controls`. Open all/Close all with focus management (focus moves to opposite button after toggle, matching GEL source nextFocusTarget pattern). Panels use `section` with `aria-labelledby`. Does not implement full controlled/uncontrolled API or exact GEL styling. Do not nest. Do not put critical information inside. |

...
| Declaration review | `27:38386` | Review step `8143:15161` | TaPaaS-specific composite | `DeclarationReview`; isolated `Declaration Review` Storybook story | Use card variant when declarations must remain visibly available. Accordion variant needs legal/content confirmation. |
...
| Legal info accordion | `22:35625` | Review step `8143:15161` | TaPaaS-specific composite using GEL accordion behaviour | `LegalInfoAccordion`; isolated `Legal Info Accordion` Storybook story | MCP confirms `BUILT` and `Custom / GEL`, but legal/privacy content needs owner and accessibility confirmation before broader use. |
...
| Confirmation page header | `9:10494` | Confirmation step `5354:8224` | GEL variant | `ConfirmationHeader`; isolated `Confirmation Header` Storybook story | Use with `role="status"` in preview. Mock completion only; needs AT review. |
...
| Details card single | `2413:787` | Later input/support/review context pages | TaPaaS-specific composite | `DetailsCard`; isolated `Details Card` Storybook story | Use as read-only context only. Do not use for decisions, identity, account truth or real routing. |
| Details card single interactive | `2958:2499` | Input/context pages, exact templates still pending | TaPaaS-specific composite | `InteractiveDetailsCard`; isolated `Interactive Details Card` Storybook story | Action semantics, focus handling and keyboard behaviour still need review before broader use. |
...
| Business error page | `8931:31271`; backend examples `31:73426` | Error routing | TaPaaS page template and pattern set | `BusinessErrorPage`, `BackendErrorExamplePage`; isolated `Business Error Page` Storybook story | Trial preview supports mock hard-stop outcomes with `role="alert"` and source-backed mock codes. Real use requires source-confirmed business rules. |
| System error page | `17628:2069` | Error routing | TaPaaS page template | `SystemErrorPage`; isolated `System Error Page` Storybook story | Use as a mock technical hard-stop surface only. Real retry, start-over, logout/session and recovery routing need app and engineering evidence. |

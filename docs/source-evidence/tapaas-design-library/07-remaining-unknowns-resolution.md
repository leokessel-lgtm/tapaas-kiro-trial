...
| MPS medical evidence source inconsistency | Resolved as preview-only evidence-status pattern, not upload behaviour. | `MpsMedicalEvidenceStatusPreview` uses `4.A - Medical certificate` `0:17316`, `4.Aa - Medical certificate uploaded` `0:17333`, static file display `0:17327`, static requirement area `0:17344` and important guidance `0:17351`. | Owner/designer confirmation of true uploaded/not-uploaded state mapping, heading text, file limits, file format rules, upload/remove behaviour, medical evidence wording and privacy/security handling. |

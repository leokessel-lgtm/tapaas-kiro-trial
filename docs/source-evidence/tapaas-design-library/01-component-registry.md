...
|---|---|---|---|---|---|
| Confirmation page header | `9:10494` | GEL variant | coded-preview | Confirmation step success header | Preview component: `ConfirmationHeader`. Isolated Storybook review and acceptance entry added for mock completion/status header semantics. |
...
| Declaration review | `27:38386` | TaPaaS-specific composite | coded-preview | Review page declaration playback | Preview component: `DeclarationReview`. Includes card and accordion variants. Isolated Storybook review and acceptance entry added. Legal content treatment still needs owner confirmation. |
...
| Details card single | `2413:787` | TaPaaS-specific composite | coded-preview | Context/details card | Preview component: `DetailsCard`. Isolated Storybook review and acceptance entry added for read-only context rows and optional local action. |
| Details card single interactive | `2958:2499` | TaPaaS-specific composite | coded-preview | Interactive context/details card | Preview component: `InteractiveDetailsCard`. Isolated Storybook review and acceptance entry added. Action model remains preview-only and needs engineering/accessibility review before broader reuse. |
...
| Legal info accordion | `22:35625` | TaPaaS-specific composite using GEL accordion behaviour | coded-preview | Legal/privacy accordion content on review or declaration pages | Preview component: `LegalInfoAccordion`. Isolated Storybook review and acceptance entry added. Figma marks it BUILT and Custom/GEL, but legal/privacy content must not be hidden or made optional without content and accessibility confirmation. |
...
| Backend error examples | `31:73426` | TaPaaS-specific pattern set | coded-preview | Backend/business/system error examples | Preview data/component: `backendErrorExamples`, `BusinessErrorPage` and `BackendErrorExamplePage` for mock-only outcomes. Isolated business error Storybook review and acceptance entry added. Real error rules and routing remain out of scope. |
| System error page | `17628:2069` | TaPaaS page template | coded-preview | Technical hard-stop system error surface | Preview component: `SystemErrorPage`. Isolated Storybook review and acceptance entry added for mock-only retry/start-again/logout action layout. Real recovery, session and routing behaviour remain out of scope. |
...
|---|---|---|---|
| `ConfirmationHeader` | `title`, `transactionName` | Confirmation page header `9:10494` | Uses a visual success mark and status semantics for mock completion headers. Isolated Storybook review and acceptance entry added. Requires accessibility review before reuse. |
...
| `BusinessErrorPage` | `title`, `message`, `guidance`, `reference`, `onStartAgain` | Business error page `8931:31271` | Source-backed trial preview for hard-stop business outcomes. Isolated Storybook review and acceptance entry added. Uses `role="alert"` around the error content. Requires real source-confirmed business rules before reuse. |
...
| `SystemErrorPage` | `title`, `message`, `guidance`, optional `reference`, optional retry/start-again/logout callbacks | System error page `17628:2069` | Mock-only technical hard-stop page. No real retry, recovery routing, logout/session, backend or operational behaviour. |
...
| `EvidenceChecklistCard` | `title`, evidence `items`, optional `children` | MPS medical frames `4.A`/`4.Aa`/`4.B`/`4.Ba`; GEL file-upload evidence | Mock-only status summary for evidence requirements. Isolated Storybook review and acceptance entry added. Does not upload files and must not be treated as GEL FileUpload. |
| `AssessmentSummaryPanel` | `title`, assessment `items`, optional `children` | MPS eligibility, concession and review frame groups; GEL status-label evidence | Mock-only routing/status display. Isolated Storybook review and acceptance entry added. Does not make decisions or validate eligibility, concessions or payments. |
| `DeclarationReview` | `title`, `intro`, `sections`, `variant` | Declaration review `27:38386` | Review-page declaration playback. Supports card and accordion variants. Isolated Storybook review and acceptance entry added. Legal content requires owner confirmation. |
...
| `LegalInfoAccordion` | `title`, `items` | Legal info accordion `22:35625` | Content-specific wrapper around GEL Accordion behaviour. Isolated Storybook review and acceptance entry added. Use only for optional legal/privacy guidance unless confirmed otherwise. |
| `InteractiveDetailsCard` | `title`, `description`, `rows`, `statusLabel`, `actions` | Details card single interactive `2958:2499` | Context card with explicit action buttons. Isolated Storybook review and acceptance entry added. Action semantics and focus expectations need review. |
...
| Details card | `DetailsCard` TaPaaS preview composite | TaPaaS-specific | coded-preview | Read-only contextual summary card. Source evidence: Details card single `2413:787`. Configurable heading level. Optional status label and action link. Isolated Storybook review and acceptance entry added. |
...
| Business error page | `BusinessErrorPage` TaPaaS preview component | TaPaaS page pattern | coded-preview | Source: Business error page `8931:31271`. Isolated Storybook review and acceptance entry added. Use only for hard-stop business outcomes with source-confirmed business rules and recovery instructions. The transaction skeleton uses simulated mock content only. |
...
| Evidence checklist card | `EvidenceChecklistCard` TaPaaS preview composition | GEL/TaPaaS composition | coded-preview | Used to summarise evidence requirements and mock statuses. Isolated Storybook review and acceptance entry added. Mapped to MPS medical evidence frames and GEL status-label/file-upload evidence. It is not a file upload component and must not claim file validation, progress, upload or storage behaviour. `MpsMedicalEvidenceStatusPreview` composes it for the MPS medical evidence status story. |
| Assessment summary panel | `AssessmentSummaryPanel` TaPaaS preview composition | GEL/TaPaaS composition | coded-preview | Used for mock routing summaries. Isolated Storybook review and acceptance entry added. Mapped to MPS eligibility/concession/review frame groups and GEL status-label evidence. It must not claim automated decisioning, eligibility assessment or policy validation. |

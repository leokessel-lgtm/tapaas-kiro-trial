...
// ---------------------------------------------------------------------------
// ProgressStepper
// GEL-aligned preview component for visual step orientation in 6+ step flows.
// Uses semantic ordered list with aria-current="step" on the active step.
...
export function ProgressStepper({ steps }: ProgressStepperProps) {
  return (
    <nav aria-label='Progress' className='gel-progress-stepper' data-gelweb-component='progress-stepper'>
      <ol style={{ display: 'flex', flexWrap: 'wrap', gap: '0', padding: 0, margin: '0 0 1.5rem', listStyle: 'none' }}>
        {steps.map((step, i) => {
          ...
          return (
            <li
              ...
              style={{
                ...
                fontFamily: 'var(--gel-font-body)', color: isCurrent ? 'var(--gel-color-text)' : 'var(--gel-color-text-grey)',
            ...
            >
              <span
                ...
                style={{
                  display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                  width: '1.5rem', height: '1.5rem', borderRadius: '50%', marginRight: '0.375rem',
                  fontSize: '0.75rem', fontWeight: 700, flexShrink: 0,
                  backgroundColor: isCompleted ? '#002664' : isCurrent ? 'var(--gel-color-primary)' : 'var(--gel-color-border)',
                  color: (isCompleted || isCurrent) ? '#fff' : 'var(--gel-color-text-grey)',
                  border: isCurrent ? '2px solid var(--gel-color-primary)' : 'none',
                }}
              >
                {isCompleted ? '✓' : i + 1}
              </span>
              ...
              {i < steps.length - 1 && (
                <span aria-hidden='true' style={{ margin: '0 0.5rem', color: 'var(--gel-color-border-dark)' }}>›</span>
              )}
            </li>
          )
        })}

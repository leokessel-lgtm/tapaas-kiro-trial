...
const stepOrder: StallStep[] = [
  'privacy', 'applicant', 'contact', 'stall', 'supporting', 'declaration', 'review', 'confirmation',
]

const stepLabels: Record<StallStep, string> = {
  privacy: 'Privacy',
  ...
  return (
    <div>
      <div className='tapaas-trial-banner'>
        <strong>TaPaaS v0.3 trial skeleton — Community stall permit.</strong>
...
function errorsForStep(step: StallStep, form: FormState) {
  ...
  if (step === 'stall') {
    if (!form.stallName.trim()) errs.push({ id: 'stall-name', text: 'Enter the stall name' })
  ...
  if (step === 'supporting') {
    if (!form.additionalInfo.trim()) errs.push({ id: 'additional-info', text: 'Provide additional information about your stall' })
  }
  ...
  return (
    <section aria-labelledby='privacy-heading'>
      ...
      <InPageAlert variant='info' title='Owner confirmation required'>
        <p>Replace this placeholder with the confirmed privacy collection notice for the community stall permit service.</p>
      </InPageAlert>
      <p>We collect your personal information to process your community stall permit application. This information may be shared with [confirmed disclosure recipients]. For more information, see [confirmed privacy policy URL].</p>
      <Checkbox
    ...
    <section aria-labelledby='contact-heading'>
      ...
      <fieldset style={{ border: 'none', padding: 0, margin: '0 0 1.5rem' }}>
        ...
        <Field id='state' label='State' hasError={stateErr} errorMessage='Select your state.'>
          <Select id='state' value={form.state} onChange={(e) => update({ state: e.target.value })} hasError={stateErr} inputWidth='md' autoComplete='address-level1' options={[
            { value: 'NSW', text: 'NSW' }, { value: 'VIC', text: 'VIC' }, { value: 'QLD', text: 'QLD' },
            { value: 'WA', text: 'WA' }, { value: 'SA', text: 'SA' }, { value: 'TAS', text: 'TAS' },
            { value: 'ACT', text: 'ACT' }, { value: 'NT', text: 'NT' },
          ]} />
        </Field>
...
function StallStep({ form, attempted, update, onBack, onContinue, onExit }: StepProps) {
  ...
  return (
    <section aria-labelledby='stall-heading'>
      ...
      <Field id='stall-name' label='Stall name' hasError={nameErr} errorMessage='Enter the stall name.'>
      ...
      <RadioButtonList
        ...
        legend='Stall type'
        options={[
          { value: 'food', label: 'Food stall (mock)' },
          { value: 'information', label: 'Information stall (mock)' },
          { value: 'merchandise', label: 'Merchandise stall (mock)' },
          { value: 'community', label: 'Community service stall (mock)' },
        ]}
        value={form.stallType}
        onChange={(v) => update({ stallType: String(v) })}
        hasError={typeErr}
...
function SupportingStep({ form, attempted, update, onBack, onContinue, onExit }: StepProps) {
  ...
  return (
    <section aria-labelledby='supporting-heading'>
      ...
      <p>Provide additional details about your stall to help us process your application. This uses mock data only.</p>
...
function ReviewStep({ form, onBack, onSubmit, onExit }: { form: FormState; onBack: () => void; onSubmit: () => void; onExit: () => void }) {
  const stallTypeLabel: Record<string, string> = { food: 'Food stall', information: 'Information stall', merchandise: 'Merchandise stall', community: 'Community service stall' }
  return (
    <section aria-labelledby='review-heading'>
      ...
      <ReviewFeesCard fees={[{ label: 'Stall application fee', amount: '$0.00' }, { label: 'Processing fee', amount: '$0.00' }]} totalAmount='$0.00' />
...
function ConfirmationStep({ form, onStartAgain }: { form: FormState; onStartAgain: () => void }) {
  ...
  return (
    <section aria-labelledby='confirmation-heading'>
      <ConfirmationHeader title='Application submitted' transactionName='Community stall permit' />
      <TransactionSummaryCard items={[
        { label: 'Reference number', value: 'STALL-000000', helpText: 'Mock reference only.' },
        { label: 'Applicant', value: form.fullName },
        { label: 'Stall', value: form.stallName },
        { label: 'Stall type', value: stallTypeLabel[form.stallType] || form.stallType },
        { label: 'Event date', value: `${form.eventDay}/${form.eventMonth}/${form.eventYear}` },
      ...
      <ol className='tapaas-step-list'>
        ...
        <li>If approved, your stall permit will be issued for the event date.</li>
        <li>Contact [confirmed support channel] if you need to change your stall details.</li>
      </ol>
      <InPageAlert variant='info' title='Owner confirmation required'>
